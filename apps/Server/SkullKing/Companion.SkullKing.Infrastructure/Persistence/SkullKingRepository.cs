...
internal class SkullKingRepository(IMongoCollection<SkullKingMatchDocument> collection) : ISkullKingMatchRepository
{
    public async Task<SkullKingMatch?> GetByMatchIdAsync(MatchId matchId, CancellationToken ct = default)
    {
    ...
    public async Task UpdateAsync(SkullKingMatch match, CancellationToken ct = default) =>
        await collection.ReplaceOneAsync(d => d.Id == match.Id.Value, ToDocument(match), cancellationToken: ct);

    private static SkullKingMatchDocument ToDocument(SkullKingMatch match) => new()
    {
        Id = match.Id.Value,
        PlayerCount = match.PlayerCount,
        TotalRounds = match.TotalRounds,
        Rounds = match.Rounds.Select(r => new RoundDocument
        {
            Id = r.Id,
            RoundNumber = r.RoundNumber,
            Status = r.Status.ToString(),
            Entries = r.Entries.Select(e => new RoundEntryDocument
            {
                Id = e.Id,
    ...
    private static SkullKingMatch ToDomain(SkullKingMatchDocument doc) =>
        SkullKingMatch.Rehydrate(
            ...
            doc.PlayerCount,
            doc.TotalRounds,
            doc.Rounds.Select(r => Round.Rehydrate(
                r.Id,
                r.RoundNumber,
                Enum.Parse<RoundStatus>(r.Status),
                r.Entries.Select(e => RoundEntry.Rehydrate(
                    e.Id,
                    new PlayerId(e.PlayerId),
                    Bid.Of(e.Bid, r.RoundNumber),
                    e.TricksWon.HasValue ? TrickCount.Of(e.TricksWon.Value, r.RoundNumber) : null,
                    e.MermaidsCaptured.HasValue ? new BonusCollection(e.MermaidsCaptured.Value, e.SkullKingCaptured!.Value, e.PiratesCapturedByMermaid!.Value, e.StandardBonuses!.Value) : null,
