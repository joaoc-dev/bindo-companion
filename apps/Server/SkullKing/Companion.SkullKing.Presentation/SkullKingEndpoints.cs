...
public static class SkullKingEndpoints
{
    public static IEndpointRouteBuilder MapSkullKingEndpoints(this IEndpointRouteBuilder routes)
    {
        ...
        sk.MapPost(
                "{matchId:guid}/initialize",
                async (Guid matchId, InitializeSkullKingRequest req, IMediator mediator) =>
                {
                    await mediator.Send(
                        new InitializeSkullKingMatchCommand(new(matchId), req.PlayerCount)
                    );
        ...
        sk.MapPost(
                "{matchId:guid}/rounds/{roundNumber:int}/bids",
                async (Guid matchId, int roundNumber, PlaceBidRequest req, IMediator mediator) =>
                {
                    await mediator.Send(
                        new PlaceBidCommand(new(matchId), roundNumber, new(req.PlayerId), req.Bid)
                    );
                    return Results.NoContent();
                }
            )
        ...
        sk.MapPost(
                "{matchId:guid}/rounds/{roundNumber:int}/results",
                async (
                    Guid matchId,
                    int roundNumber,
                ...
                ) =>
                {
                    var score = await mediator.Send(
                        new SubmitRoundResultCommand(
                            new(matchId),
                            roundNumber,
                            new(req.PlayerId),
                            req.TricksWon,
                            req.MermaidsCaptured,
                            req.SkullKingCaptured,
                            req.PiratesCapturedByMermaid,
                            req.StandardBonuses
                        )
                    );
        ...
        sk.MapPost(
                "{matchId:guid}/rounds/{roundNumber:int}/finalize",
                async (Guid matchId, int roundNumber, IMediator mediator) =>
                {
        ...
        sk.MapGet(
                "{matchId:guid}/scoreboard",
                async (Guid matchId, IMediator mediator) =>
                {
